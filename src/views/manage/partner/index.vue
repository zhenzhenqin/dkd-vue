<template>
      ...
      <el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" @click="resetPwd(scope.row)"
            v-hasPermi="['manage:region:query']">重置密码</el-button>
          <el-button link type="primary" @click="getPartnerInfo(scope.row)"
            v-hasPermi="['manage:region:query']">查看详情</el-button>
  ...
  <!-- 查看合作商详情对话框 -->
  <el-dialog title="合作商详情" v-model="partnerInfo" width="500px" append-to-body>
...
//查看合作商详情
const partnerInfo = ref(false)
function getPartnerInfo(row) {
  reset();
  const _id = row.id
  getPartner(_id).then(response => {
    form.value = response.data;
    partnerInfo.value = true;
  });
}
